...
import { useCompatibilityColor } from '../../design-system/constants/status'
...
export default function SuggestionCard({ id, name, location, gustos = [], compatibility = 0, favoritesCount = 0, onConnect, onView }: Props) {
  ...
  return (
    <View style={[styles.card, { borderColor: colors.borderLight, backgroundColor: colors.surface }]}> 
      <View style={styles.requestHeader}>
        ...
        <View style={[styles.compatibilityBadge, { backgroundColor: getCompatibilityColor(compatibility) }]}>
          <Text style={[styles.compatibilityText, { color: colors.textOnPrimary }]}>{compatibility}%</Text>
        </View>
      </View>
      ...
      <View style={{marginTop:12}}>
        ...
        <Text style={[styles.smallText, { color: colors.textSecondary }]}>{favoritesCount} restaurantes favoritos</Text>
        <View style={styles.requestActions}>
          <TouchableOpacity style={[styles.outlinedButton, { borderColor: colors.borderLight }]} onPress={() => onView?.(id)}>
            <Icon name="person" size={16} color={colors.warning} />
            <Text style={styles.outlinedLabel}>Ver perfil</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.primaryButton, { backgroundColor: colors.warning }]} onPress={() => onConnect?.(id)}>
            <Icon name="person-add" size={16} color={colors.textOnPrimary} />
            <Text style={[styles.primaryLabel, { color: colors.textOnPrimary }]}>Conectar</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  smallText: { fontSize:12, marginTop:8 },
