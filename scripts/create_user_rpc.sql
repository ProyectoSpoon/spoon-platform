-- ========================================
-- FUNCIÓN RPC PARA CREAR USUARIOS DIRECTOS
-- Soluciona el problema RLS que impide crear usuarios
-- ========================================

-- Función RPC que crea usuarios sin restricciones RLS
...
BEGIN
  ...
  -- NOTA: No asignamos restaurant_id aquí para evitar constraint único
  INSERT INTO users (
    id,
    email,
    first_name,
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    -- Nota: En PostgreSQL, las funciones son atómicas por defecto
...
GRANT EXECUTE ON FUNCTION create_user_direct(text, text, text, text, text, uuid) TO authenticated;
...
BEGIN
  ...
  IF p_restaurant_id IS NULL THEN
    RAISE EXCEPTION 'Restaurant ID es obligatorio';
  END IF;

  IF p_name IS NULL OR p_name = '' THEN
    RAISE EXCEPTION 'Nombre del rol es obligatorio';
  END IF;

  -- Verificar que no exista un rol con el mismo nombre en el restaurante
  IF EXISTS (
    SELECT 1 FROM custom_roles
    WHERE restaurant_id = p_restaurant_id
    AND name = p_name
  ) THEN
    RAISE EXCEPTION 'Ya existe un rol con este nombre en el restaurante';
  END IF;
  ...
  RETURN json_build_object(
    ...
    'role_id', v_role_id,
    'name', p_name,
    'permissions_assigned', array_length(v_permission_ids, 1)
  );

EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'Error creando rol personalizado: %', SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Otorgar permisos
GRANT EXECUTE ON FUNCTION create_custom_role(uuid, text, text, text[]) TO authenticated;
...
BEGIN
  ...
  ORDER BY sr.name; -- Ordenar por nombre en lugar de display_order

  -- Roles personalizados del restaurante (si se especifica)
...
COMMENT ON FUNCTION create_user_direct(text, text, text, text, text, uuid) IS
'Solucion crítica: Crea usuarios directamente sin restricciones RLS. Genera contraseña temporal segura.';

COMMENT ON FUNCTION create_custom_role(uuid, text, text, text[]) IS
'Crea roles personalizados con permisos específicos para un restaurante.';

COMMENT ON FUNCTION get_available_roles(uuid) IS
'Retorna todos los roles disponibles: sistema + personalizados del restaurante.';
